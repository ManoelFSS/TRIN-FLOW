...
const RADIUS_EARTH_METERS = 6378137;
const DEFAULT_CENTER = [-12.432558, -51.772750];
const ZOOM = 4;
...
const Tracking = () => {
  ...
  // Geolocalização com watchPosition otimizado
  useEffect(() => {
    let watchId;
  ...
  const toDeg = useCallback((value) => (value * 180) / Math.PI, []);

  const moveTowards = useCallback((currentLat, currentLng, targetLat, targetLng, distanceMeters) => {
    const dLat = toRad(targetLat - currentLat);
    const dLng = toRad(targetLng - currentLng);
    ...
    const a = Math.sin(dLat / 2) ** 2 +
              ...
              Math.sin(dLng / 2) ** 2;
    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
    const distance = RADIUS_EARTH_METERS * c;

    if (distance < distanceMeters) return { lat: targetLat, lng: targetLng };
  ...
  const calculateDirection = useCallback((curLat, curLng, tgtLat, tgtLng, rotation) => {
    const diffLat = tgtLat - curLat;
    const diffLng = tgtLng - curLng;
    const angleRad = Math.atan2(diffLng, diffLat);
    let angleDeg = toDeg(angleRad);

    let delta = angleDeg - rotation;
    if (Math.abs(delta) > 180) {
      angleDeg += delta > 0 ? -360 : 360;
    }

    return (angleDeg + 360) % 360;
  }, [toDeg]);

  // Atualização periódica da posição dos veículos
  useEffect(() => {
    const interval = setInterval(() => {
      setVehicles((prev) =>
        prev.map((v) => {
          ...
          const next = moveTowards(v.latitude, v.longitude, currentLocation.latitude, currentLocation.longitude, MOVE_DISTANCE);
          const rotation = calculateDirection(v.latitude, v.longitude, next.lat, next.lng, v.rotation);

          return { ...v, latitude: next.lat, longitude: next.lng, rotation };
        })
      );
    }, 300); // 300ms é mais razoável que 20ms para web

    return () => clearInterval(interval);
  }, [currentLocation, moveTowards, calculateDirection]);

  // Ícone memoizado para performance
  const createVehicleIcon = useCallback((rotation) =>
    new L.DivIcon({
      html: `<div class="vehicle-icon" style="
        transition: transform 0.3s linear;
        width: 25px;
        height: 50px;
        background: url(${CartRight}) no-repeat center center / cover;
        transform: rotate(${rotation}deg);
        user-select: none;
        pointer-events: none;
      "></div>`,
  ...
  const safeGeoJSON = useMemo(() => americaDoSul || { type: "FeatureCollection", features: [] }, []);
    ...
    <Container_tracking>
      ...
      >
        <TileLayer
          ...
          attribution='© <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        />
        <GeoJSON data={safeGeoJSON} style={{ fillColor: "transparent", weight: 0, color: "blue", opacity: 1, fillOpacity: 0 }} />
        {vehicles.map((v) => (
          <Marker key={v.id} position={[v.latitude, v.longitude]} icon={createVehicleIcon(v.rotation)}>
            <Popup>
